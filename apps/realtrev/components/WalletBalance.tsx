    ...
    const balanceTimer = setInterval(() => {
      setBalance(prev => {
        if (prev < targetBalance) {
          return Math.min(prev + 10, targetBalance)
        }
      ...
      <CardContent className="p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <div className="text-4xl font-bold flex items-center">
              ...
              <span className="mr-2">{balance.toLocaleString()}</span>
              <span className="text-xl font-normal">Trev Coins</span>
            </div>
          </div>
          <div className="text-right">
            <p className="text-sm opacity-75">Your Travel Rewards at a Glance</p>
          </div>
        </div>
        ...
        <div className="mt-4">
          ...
          <div className="relative">
            ...
            <Plane className="h-4 w-4 text-white absolute top-1/2 -translate-y-1/2 transition-all duration-300 ease-in-out" style={{ left: `${planePosition}%` }} />
          </div>
        </div>
      </CardContent>
