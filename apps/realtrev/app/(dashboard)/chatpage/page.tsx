...
const mockMessages = [
  ...
  { id: 2, sender: 'Traveler', name: 'Alex', message: 'Hi Emma! I\'m looking for some local food recommendations', avatar: '/placeholder.svg?height=40&width=40' },
...
export default function Component() {
  ...
  const [location, setLocation] = useState(''); // To store location for joining
  ...
  const handleSendMessage = () => {
    if (inputMessage.trim() && location.trim()) {
      const newMessage = {
        id: messages.length + 1,
        sender: 'Traveler',
        name: 'Alex', // Update this to get the actual traveler's name
        message: inputMessage,
        avatar: '/placeholder.svg?height=40&width=40',
      };
      setMessages([...messages, newMessage]);
      socket.emit('joinLocation', location.trim()); // Join the location room
      socket.emit('sendMessage', { location: location.trim(), message: inputMessage.trim() }); // Send message
      setInputMessage(''); // Clear input field
    } else {
      alert("Please enter both location and message."); // Alert for empty fields
    }
  };
