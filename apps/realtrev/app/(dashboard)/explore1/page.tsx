...
const LiveMap = dynamic(() => import("@/app/components/map"), {
  ssr: false,
...
export default function ExplorePage() {
  ...
  const setmapQuery = useRecoilState(mapQueryState)[1];
  useEffect(() => {
  ...
  useEffect(() => {
    const fetchAndSetUpdates = async () => {
      ...
      if (Array.isArray(updates)) {
        const initialLikesDislikes = updates.reduce(
          (acc, update) => {
            acc[update.id] = {
              likes: Number(update.likes),
              dislikes: Number(update.dislikes),
            };
            return acc;
          },
  ...
  const sortedTrendingUpdates = useMemo(
    () =>
  ...
  const handleClickquery = useCallback(
    (queryLocation: any) => {
      const newlocationrreverse = {
        coordinates: [queryLocation[1], queryLocation[0]],
      };
      setmapQuery({
        trigger: true,
        location: newlocationrreverse.coordinates as [number, number],
      });
    },
    [setmapQuery]
  );
...
function UpdateCard({
  ...
  likes: number;
  dislikes: number;
  setLikesDislikes: React.Dispatch<
    React.SetStateAction<{ [key: number]: { likes: number; dislikes: number } }>
  >;
  coordinates: any;
  setmapQuery: any;
}) {
        ...
        <div className="flex justify-between items-start mb-2">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              ...
              <span
                className={`block text-xs font-semibold ${update.type === "HIGH" ? "text-red-500" : update.type === "MEDIUM" ? "text-yellow-500" : "text-green-500"}`}
              >
                {update?.type}
              </span>
            </p>
